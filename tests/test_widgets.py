...
def test_select_mixed_grouped_and_ungrouped_choices(dummy_field_class):
    ...
    field = dummy_field_class(
        groups=[
            ("g1", [Choice("foo", "lfoo", selected=True)]),
            ("g2", [Choice("baz", "lbaz")]),
            (None, [Choice("abc", "labc")]),
            ("g1", [Choice("bar", "lbar")]),
            (None, [Choice("xyz", "lxyz")]),
        ]
    )
    field.name = "f"
...
def test_render_option():
    assert (
        Select.render_option(Choice("bar", "foo", selected=False))
        == '<option value="bar">foo</option>'
    )

    assert (
        Select.render_option(Choice(True, "foo", selected=True))
        == '<option selected value="True">foo</option>'
    )

    assert (
        Select.render_option(Choice(False, "foo", selected=False))
        == '<option value="False">foo</option>'
    )

    assert (
        Select.render_option(Choice("bar", '<i class="bar"></i>foo', selected=False))
        == '<option value="bar">&lt;i class=&#34;bar&#34;&gt;&lt;/i&gt;foo</option>'
    )

    assert (
        Select.render_option(
            Choice("bar", Markup('<i class="bar"></i>foo'), selected=False)
        )
        == '<option value="bar"><i class="bar"></i>foo</option>'
    )
