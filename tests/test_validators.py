...
class ValidatorsTest(TestCase):
    ...
    def test_email(self):
        ...
        self.assertRaises(
            ValidationError, email(), self.form, DummyField("foo@bar.ab12")
        )
        self.assertRaises(
            ValidationError, email(), self.form, DummyField("foo@.bar.ab")
        )
        self.assertRaises(
            ValidationError, email(), self.form, DummyField("foo.@bar.co")
        )
        self.assertRaises(
            ValidationError, email(), self.form, DummyField("foo@foo@bar.co")
        )
        self.assertRaises(
            ValidationError, email(), self.form, DummyField("fo o@bar.co")
        )
    ...
    def test_equal_to(self):
        ...
        self.assertRaises(
            ValidationError,
            equal_to("invalid_field_name"),
            self.form,
            DummyField("test"),
        )
        self.assertRaises(
            ValidationError, equal_to("foo"), self.form, DummyField("different_value")
        )

    def test_ip_address(self):
        self.assertEqual(ip_address()(self.form, DummyField("127.0.0.1")), None)
        self.assertRaises(
            ValidationError, ip_address(), self.form, DummyField("abc.0.0.1")
        )
        self.assertRaises(
            ValidationError, ip_address(), self.form, DummyField("1278.0.0.1")
        )
        self.assertRaises(
            ValidationError, ip_address(), self.form, DummyField("127.0.0.abc")
        )
        self.assertRaises(
            ValidationError, ip_address(), self.form, DummyField("900.200.100.75")
        )
        for bad_address in (
            "abc.0.0.1",
            "abcd:1234::123::1",
    ...
    def test_uuid(self):
        self.assertEqual(
            UUID()(self.form, DummyField("2bc1c94f-0deb-43e9-92a1-4775189ec9f8")), None
        )
        self.assertEqual(
            UUID()(self.form, DummyField("2bc1c94f0deb43e992a14775189ec9f8")), None
        )
        self.assertRaises(
            ValidationError,
            UUID(),
    ...
    def test_data_required(self):
        ...
        grab = lambda **k: grab_stop_message(
            data_required(**k), self.form, DummyField("")
        )
    ...
    def test_input_required(self):
        self.assertEqual(
            input_required()(self.form, DummyField("foobar", raw_data=["foobar"])), None
        )
        self.assertRaises(
            StopValidation, input_required(), self.form, DummyField("", raw_data=[""])
        )
        self.assertEqual(input_required().field_flags, ("required",))

        # Check message and custom message
        grab = lambda **k: grab_stop_message(
            input_required(**k), self.form, DummyField("", raw_data=[""])
        )
        self.assertEqual(grab(), "This field is required.")
        self.assertEqual(grab(message="foo"), "foo")
    ...
    def test_optional(self):
        self.assertEqual(
            optional()(self.form, DummyField("foobar", raw_data=["foobar"])), None
        )
        self.assertRaises(
            StopValidation, optional(), self.form, DummyField("", raw_data=[""])
        )
    ...
    def test_regexp(self):
        ...
        self.assertEqual(
            regexp("^a", re.I)(self.form, DummyField("ABcd")).group(0), "A"
        )
        ...
        # Compiled regexp
        self.assertEqual(
            regexp(re.compile("^a"))(self.form, DummyField("abcd")).group(0), "a"
        )
        self.assertEqual(
            regexp(re.compile("^a", re.I))(self.form, DummyField("ABcd")).group(0), "A"
        )
        self.assertRaises(
            ValidationError, regexp(re.compile("^a")), self.form, DummyField("foo")
        )
        self.assertRaises(
            ValidationError, regexp(re.compile("^a")), self.form, DummyField(None)
        )

        # Check custom message
        self.assertEqual(
            grab_error_message(regexp("^a", message="foo"), self.form, DummyField("f")),
            "foo",
        )
    ...
    def test_url(self):
        ...
        self.assertEqual(url()(self.form, DummyField("http://foobar.dk/")), None)
        self.assertEqual(
            url()(self.form, DummyField("http://foobar.museum/foobar")), None
        )
        self.assertEqual(url()(self.form, DummyField("http://127.0.0.1/foobar")), None)
        self.assertEqual(
            url()(self.form, DummyField("http://127.0.0.1:9000/fake")), None
        )
        self.assertEqual(
        ...
        self.assertEqual(
            url(require_tld=False)(self.form, DummyField("http://foobar")), None
        )
        self.assertRaises(
            ValidationError, url(), self.form, DummyField("http://foobar")
        )
        self.assertRaises(ValidationError, url(), self.form, DummyField("foobar.dk"))
        self.assertRaises(
            ValidationError, url(), self.form, DummyField("http://127.0.0/asdf")
        )
        self.assertRaises(
            ValidationError, url(), self.form, DummyField("http://foobar.d")
        )
        self.assertRaises(
            ValidationError, url(), self.form, DummyField("http://foobar.12")
        )
        self.assertRaises(
            ValidationError, url(), self.form, DummyField("http://localhost:abc/a")
        )
        # Test IDNA
        IDNA_TESTS = (
    ...
    def test_any_of(self):
        ...
        self.assertRaises(
            ValueError, AnyOf(["a", "b", "c"]), self.form, DummyField(None)
        )
        ...
        checker = AnyOf(
            [7, 8, 9], message="test %(values)s", values_formatter=formatter
        )
        self.assertEqual(
    ...
    def test_none_of(self):
        ...
        self.assertRaises(
            ValueError, NoneOf(["a", "b", "c"]), self.form, DummyField("a")
        )
